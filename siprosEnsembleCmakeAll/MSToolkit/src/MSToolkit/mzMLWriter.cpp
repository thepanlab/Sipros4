...
bool MzMLWriter::writeIndex()
{
  ...
  if (vIndex.size() > 0)
  {
    ...
    {
      if (!exportOffset(vIndex[i].id, vIndex[i].offset, 3))
        return false;
    }
    if (bTabs)
    {
  ...
  if (vChromIndex.size() > 0)
  {
    ...
    {
      if (!exportOffset(vChromIndex[i].id, vChromIndex[i].offset, 3))
        return false;
    }
    if (bTabs)
    {
...
bool MzMLWriter::exportBinaryDataArray(mzParser::BasicChromatogram &c, bool bRT, int tabs)
{
  ...
  else
  {
    ...
    {
      for (i = 0; i < c.size(); i++)
        d.push_back(c[i].intensity);
    }
    else
    {
      for (i = 0; i < c.size(); i++)
        f.push_back((float)c[i].intensity);
    }
  }
  ...
  if (bZlib)
  {
    ...
    {
      if (bRT)
        len = (uLong)d.size() * sizeof(double);
      else
        len = (uLong)f.size() * sizeof(float);
      zLen = compressBound(len);
      zCompr = (unsigned char *)calloc((uInt)zLen, 1);
      if (bRT)
        compress(zCompr, &zLen, (const Bytef *)&d[0], len);
      else
        compress(zCompr, &zLen, (const Bytef *)&f[0], len);
    }
  }
  ...
  else
  {
    ...
    {
      if (bRT)
        sz64 = d.size() * sizeof(double);
      else
        sz64 = f.size() * sizeof(float);
    }
  }
  i = sz64 % 3;
  if (i > 0)
  ...
  if (bNumpress)
  {
    if (bZlib)
      ...
      i = mzParser::b64_encode(arr64, (char *)numpr, (int)numprLen);
  }
  else
  {
    if (bZlib)
    ...
    {
      if (bRT)
        i = mzParser::b64_encode(arr64, (char *)&d[0], (int)(d.size() * sizeof(double)));
      else
        i = mzParser::b64_encode(arr64, (char *)&f[0], (int)(f.size() * sizeof(float)));
    }
  }
  ...
  if (bZlib)
    exportCvParam("MS:1000574", "MS", "zlib compression", "", "", "", "", tabs + 1);
  if (bNumpress)
  {
    if (bRT)
      exportCvParam("MS:1002312", "MS", "MS-Numpress linear prediction compression", "", "", "", "", tabs + 1);
    else
      exportCvParam("MS:1002314", "MS", "MS-Numpress short logged float compression", "", "", "", "", tabs + 1);
  }
  else
  {
    if (bRT)
      exportCvParam("MS:1000523", "MS", "64-bit float", "", "", "", "", tabs + 1);
    else
      exportCvParam("MS:1000521", "MS", "32-bit float", "", "", "", "", tabs + 1);
  }
  if (bRT)
    exportCvParam("MS:1000595", "MS", "time array", "UO:0000010", "MS", "second", "", tabs + 1);
  else
    exportCvParam("MS:1000515", "MS", "intensity array", "MS:1000131", "MS", "number of detector counts", "", tabs + 1);
  if (!exportBinary(arr64, i, tabs + 1))
    return false;
  if (bTabs)
    fprintf(fptr, "%s", &tbs[0]);
...
bool MzMLWriter::exportBinaryDataArray(Spectrum &s, bool bMZ, int tabs)
{
  ...
  else
  {
    ...
    {
      for (i = 0; i < s.size(); i++)
        d.push_back(s[i].intensity);
    }
    else
    {
      for (i = 0; i < s.size(); i++)
        f.push_back(s[i].intensity);
    }
  }
  ...
  if (bMZ)
    exportCvParam("MS:1000514", "MS", "m/z array", "MS:1000040", "MS", "m/z", "", tabs + 1);
  else
    exportCvParam("MS:1000515", "MS", "intensity array", "MS:1000131", "MS", "number of detector counts", "", tabs + 1);
  if (!exportBinary(arr64, i, tabs + 1))
    return false;
...
bool MzMLWriter::exportBinaryDataArrayList(mzParser::BasicChromatogram &c, int tabs)
{
  ...
  if (!exportBinaryDataArray(c, true, tabs + 1))
    return false;
  if (!exportBinaryDataArray(c, false, tabs + 1))
    return false;
  if (bTabs)
    fprintf(fptr, "%s", &tbs[0]);
...
bool MzMLWriter::exportBinaryDataArrayList(Spectrum &s, int tabs)
{
  ...
  fprintf(fptr, "<binaryDataArrayList count=\"2\">\n");
  if (!exportBinaryDataArray(s, true, tabs + 1))
    return false;
  if (!exportBinaryDataArray(s, false, tabs + 1))
    return false;
  if (bTabs)
    fprintf(fptr, "%s", &tbs[0]);
  fprintf(fptr, "</binaryDataArrayList>\n");
...
bool MzMLWriter::exportChromatogram(mzParser::BasicChromatogram &c, int tabs)
{
  ...
  if (!exportPrecursor(c, tabs + 1))
    return false;
  if (c.getProdMZ() > 0)
  {
    if (!exportProduct(c, tabs + 1))
      return false;
  }
  if (!exportBinaryDataArrayList(c, tabs + 1))
    return false;
...
bool MzMLWriter::exportCvParam(string ac, string ref, string name, string unitAc, string unitRef, string unitName, string value, int tabs)
{
  ...
  if (value.size() > 0)
    ex += " value=\"" + value + "\"";
  if (unitRef.size() > 0)
    ex += " unitCvRef=\"" + unitRef + "\"";
  if (unitAc.size() > 0)
    ex += " unitAccession=\"" + unitAc + "\"";
  if (unitName.size() > 0)
    ex += " unitName=\"" + unitName + "\"";
  ex += "/>";
  fprintf(fptr, "%s\n", &ex[0]);
  return true;
}
...
bool MzMLWriter::exportIsolationWindow(mzParser::BasicChromatogram &c, bool bPre, int tabs)
{
  ...
  if (bPre)
    sprintf(tmp, "%.4lf", c.getPreMZ());
  else
    sprintf(tmp, "%.4lf", c.getProdMZ());
  value = tmp;
  exportCvParam("MS:1000827", "MS", "isolation window target m/z", "MS:1000040", "MS", "m/z", value, tabs + 1);
  if (bPre)
    sprintf(tmp, "%.4lf", c.getPreOffsetLower());
  else
  ...
  if (bPre)
    sprintf(tmp, "%.4lf", c.getPreOffsetUpper());
  else
    sprintf(tmp, "%.4lf", c.getProdOffsetUpper());
  value = tmp;
...
bool MzMLWriter::exportPrecursor(mzParser::BasicChromatogram &c, int tabs)
{
  ...
  if (!exportIsolationWindow(c, true, tabs + 1))
    return false;
  if (c.getPreMZ() > 0)
  {
    if (!exportSelectedIonList(c, tabs + 1))
      return false;
  }
...
bool MzMLWriter::exportPrecursor(Spectrum &s, int tabs)
{
  ...
  fprintf(fptr, "<precursor>\n");
  if (!exportIsolationWindow(s, tabs + 1))
    return false;
  // if(s.getMonoMZ()>0) {
  if (!exportSelectedIonList(s, tabs + 1))
    return false;
  //}
  if (!exportActivation(s, tabs + 1))
    return false;
...
bool MzMLWriter::exportProduct(mzParser::BasicChromatogram &c, int tabs)
{
  ...
  if (!exportIsolationWindow(c, false, tabs + 1))
    return false;
  if (bTabs)
    exportTabs(tabs);
  fprintf(fptr, "</product>\n");
...
bool MzMLWriter::exportScan(Spectrum &s, int tabs)
{
  ...
  if (!exportScanWindowList(s, tabs + 1))
    return false;
...
bool MzMLWriter::exportSpectrum(Spectrum &s, int tabs)
{
  ...
  if (!exportScanList(s, tabs + 1))
    return false;
  if (s.getMsLevel() > 1)
  {
    if (!exportPrecursorList(s, tabs + 1))
      return false;
  }
  if (!exportBinaryDataArrayList(s, tabs + 1))
    return false;
